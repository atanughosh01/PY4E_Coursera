...
# Enter http://py4e-data.dr-chuck.net/known_by_Bob.html as the URL
...
except Exception as e:                                          # catch exception upon type-casting failure
    print(e, "\nPlease Enter Numeric Integer Value(s)")         # display error message for not entering int data
    quit()                                                      # quits the program execution

for i in range(repeat_count):                                   # do until repetition count != 0
    try:                                                        # try to open URL, parse HTML, and fetch all NAME(s)
...
print("\nThe Last Name Found is :", name)                       # display the final result // name
