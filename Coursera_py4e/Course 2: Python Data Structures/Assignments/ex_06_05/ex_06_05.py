...
if pos1 >= 0:  # condition if the sub-string is present (doesn't return -1)
    print('The sub-string',find_str,'is present at position =',pos1,'and ending at position =',pos2 - 1)
else:  # condition if the sub-string is absent (returns -1)
    print('ERROR! The sub-string is not present in the original string')
    quit()

new_text = text[pos1:pos2]  # slicing the string
try:
    num = float(new_text)
